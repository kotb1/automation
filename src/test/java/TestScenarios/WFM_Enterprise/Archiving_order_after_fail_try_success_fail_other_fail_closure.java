...
public class Archiving_order_after_fail_try_success_fail_other_fail_closure extends TestBase {
    ...
    public void order_Creation(String URL,String username, String password,String request_type) throws Exception {
        ...
        body = create.update_complain_number();
    ...
    @Test(dependsOnMethods = "validating_closure_attributes")
    @Parameters({"username"})
    public void reopen_WO(String username) throws Exception {
        Assert.assertTrue(Data_base.update_close_ready_to_completed(Work_order_id));
        Assert.assertTrue(Data_base.archive_work_order(Work_order_id,username));
        reopen=new Reopen_Ticket();
//        System.out.println(body);
        String new_WO_id= reopen.reopen_archived_ticket(body);
        Assert.assertNotEquals(new_WO_id,Work_order_id);
